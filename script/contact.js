...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Validation et soumission du formulaire avec reCAPTCHA
    ...
    if (contactForm) {
        contactForm.addEventListener('submit', async function (e) {
            ...
            try {
                ...
                const token = await grecaptcha.enterprise.execute(RECAPTCHA_SITE_KEY, { action: 'contact_submit' });
                formData.append('recaptcha_token', token);
                formData.append('recaptcha_action', 'contact_submit');

                // Envoyer les données au serveur
                submitBtn.textContent = 'Envoi en cours...';
                const response = await fetch('../script/process_contact.php', {
                    method: 'POST',
                    body: formData
                });
